...
static u8 work_modle = 0;

void UpdateUI(void);                                                                 //更新UI数据
					
void set_work_model(u8 model)
{
		if(model==1)
		{
			   bat_val1=1300.00;
    ...
    {        
             ...
             SetButtonValue(10,5,1);                                                      //隐藏
             AnimationPlayFrame(10,1,3);   
		}
		
						Dac1_Set_Vol(bat_val1);																													//设置电压表和电流表
...
void OPERATION_LOCAL_MODLE(void)
{
	...
	else
	{
		...
		*/
		...
		if(OPEN_GATE==0)      	//定时器 5S  ENERGY_STATE为真
		{
					DelayMS(200);
...
void OPERATION_DISTANCE_MODLE(void)
{
		...
		if((Secondary_switch==0) && (ENERGY==0))		// 远地模式二级开发
		{
...
{ 
  ...
  systicket_init();      
	UartInit(19200);		
	TIM3_Init(50000-1,7200 - 1);  //   7200 0000 /7200 = 10000 hz = 0.0001s = 0.1ms 
	cabinet_gpio_init();		 				 				
	Dac_Init();		 			
	...
	while(1)
	{
      ...
      {                                                                           
			...
			{	
				...
				if((current_screen_id != 10) && work_modle)
				{
						switch(t%3)
						{
	  ...
	  if(screen_id == 3)
    {
		...
		if(screen_id == 10)
		{
			 ...
			 {	 
					switch(control_id)
					{
						case 2:	set_work_model(1);work_modle = 0;break;     								//正常运行
						case 3:	set_work_model(2);work_modle = 0;break; 										//过压设置  
						case 4:	set_work_model(3);work_modle = 0;break;		 									//过流设置    
						case 5:	work_modle = 1;break; 																			//循环变化
						default:break;
					}
...
{                                                             
    ...
    {            
        ...
        {                                                                            
            ...
            sprintf((char*)back,"%c",(255-value*2));                                        //设置背光亮度 背光值范围 0~255，0最亮，255最暗
            SetBackLight(back[0]);
        }
    }
